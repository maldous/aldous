...
k8s_yaml('k8s/generated/redis-commander.yaml')
...
k8s_resource('keycloak', resource_deps=['deploy-pg-cluster', 'minio', 'redis-master'], port_forwards=['8080:8080'])
...
k8s_resource('kong-kong', resource_deps=['kong-kong-post-upgrade-migrations', 'kong-crds'], port_forwards=['8001:8001', '8002:8002'])
k8s_resource('loki')
k8s_resource('tempo')
k8s_resource('minio', port_forwards=['9001:9001'])
k8s_resource('prometheus-server', new_name='prometheus', port_forwards=['9090:9090'])
k8s_resource('prometheus-kube-state-metrics')
k8s_resource('prometheus-prometheus-node-exporter', new_name='node-exporter')
k8s_resource('grafana', resource_deps=['loki','tempo','prometheus'], port_forwards=['3000:3000'])
k8s_resource('alloy')
k8s_resource('mailhog', port_forwards=['8025:8025'])
k8s_resource('meilisearch', port_forwards=['7700:7700'])
k8s_resource('redis-commander', resource_deps=['redis-master'], port_forwards=['8081:8081'])
