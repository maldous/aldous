...
# Destructive cleanup
clean:
	@echo "-> Destroying cluster (one-way!)"
	@if [[ "${CONFIRM_RESET:-}" != "YES" ]]; then \
		echo "DANGER: this will destroy MicroK8s, Ceph, etc."; \
		echo "Export CONFIRM_RESET=YES to proceed."; \
		exit 1; \
	fi
	@set -x; \
	...
	sudo killall -9 systemd-udevd udevd || true; \
...
help:
	...
	@echo "  clean - Destroy entire cluster (DESTRUCTIVE)"
